...
function bigbluebuttonbn_get_cfg_server_url($server = null) {
    global $BIGBLUEBUTTONBN_CFG, $CFG;

    if (!empty($server)) {
		if (!empty($BIGBLUEBUTTONBN_CFG->bigbluebuttonbn_cluster[$server])) {
			return trim(trim($BIGBLUEBUTTONBN_CFG->bigbluebuttonbn_cluster[$server]->server_url),'/').'/';
		}
    } elseif (isset($BIGBLUEBUTTONBN_CFG->bigbluebuttonbn_server_url)) {
    	return trim(trim($BIGBLUEBUTTONBN_CFG->bigbluebuttonbn_server_url),'/').'/';
    } elseif (isset($CFG->bigbluebuttonbn_server_url)) {
    	return trim(trim($CFG->bigbluebuttonbn_server_url),'/').'/';
    }

    return 'http://test-install.blindsidenetworks.com/bigbluebutton/';
}

function bigbluebuttonbn_get_cfg_shared_secret($server = null) {
    global $BIGBLUEBUTTONBN_CFG, $CFG;

    if (!empty($server)) {
	    if (!empty($BIGBLUEBUTTONBN_CFG->bigbluebuttonbn_cluster[$server])) {
		    return $BIGBLUEBUTTONBN_CFG->bigbluebuttonbn_cluster[$server]->shared_secret;
	    }
    } elseif (isset($BIGBLUEBUTTONBN_CFG->bigbluebuttonbn_shared_secret)) {
    	return trim($BIGBLUEBUTTONBN_CFG->bigbluebuttonbn_shared_secret);
    } elseif (isset($CFG->bigbluebuttonbn_shared_secret)) {
